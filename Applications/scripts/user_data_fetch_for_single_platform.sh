...
if [ -f ../tmp/adminmail_user_data_fetch_for_single_platform.sh.lock ]; then
 exit 0;
fi
...
mysql -u logintracking -p$DBPASS logintracking -e "select userName from login where platform='$PLATFORM';" > ../tmp/user_fetch_$FILENAME.txt
...
echo "ALL User data for specific Month, see attached file !" | mailx -s "ALL User data for specific Month, see attached file !" -r Login-Management-System@list.lilly.com -a ../tmp/user_data_fetch_$FILENAME.txt $EMAILID
...
touch ../tmp/adminmail_user_data_fetch_for_single_platform.sh.lock
sleep 5
rm -rf ../tmp/adminmail_user_data_fetch_for_single_platform.sh.lock
